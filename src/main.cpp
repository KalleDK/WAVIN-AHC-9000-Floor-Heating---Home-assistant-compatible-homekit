...
#define LED1 D5     // Led in PULSE function
#define LED2 D2     // Led in PULSE function
...
void publishConfiguration(uint8_t channel)
{
  ...
  String climateMessage = String(
      ...
      MQTT_PREFIX + mqttDeviceNameWithMac + "/" + channel + MQTT_SUFFIX_CURRENT + "\", "
                                                                                  "\"temperature_command_topic\": \"" +
      MQTT_PREFIX + mqttDeviceNameWithMac + "/" + channel + MQTT_SUFFIX_SETPOINT_SET + "\", "
                                                                                       "\"temperature_state_topic\": \"" +
      MQTT_PREFIX + mqttDeviceNameWithMac + "/" + channel + MQTT_SUFFIX_SETPOINT_GET + "\", "
                                                                                       "\"mode_command_topic\": \"" +
      MQTT_PREFIX + mqttDeviceNameWithMac + "/" + channel + MQTT_SUFFIX_MODE_SET + "\", "
                                                                                   "\"mode_state_topic\": \"" +
      MQTT_PREFIX + mqttDeviceNameWithMac + "/" + channel + MQTT_SUFFIX_MODE_GET + "\", "
                                                                                   "\"modes\": [\"" +
      MQTT_VALUE_MODE_MANUAL + "\", \"" + MQTT_VALUE_MODE_STANDBY + "\"], "
                                                                    "\"availability_topic\": \"" +
      MQTT_PREFIX + mqttDeviceNameWithMac + MQTT_ONLINE + "\", "
  ...
  String batteryMessage = String(
      "{\"name\": \"" + mqttDeviceNameWithMac + "_" + channel + "_battery\", "
                                                                "\"state_topic\": \"" +
      MQTT_PREFIX + mqttDeviceNameWithMac + "/" + channel + "/battery\", "
                                                            "\"availability_topic\": \"" +
      MQTT_PREFIX + mqttDeviceNameWithMac + MQTT_ONLINE + "\", "
                                                          "\"payload_available\": \"True\", "
                                                          "\"payload_not_available\": \"False\", "
...
void setup()
{
  ...
  pinMode(LED1, OUTPUT); // LED connected on PIN D5 as output (MQTT Indicator).

  pinMode(LED2, OUTPUT); // LED connected on PIN D2 as output (WIFI Indicator).

  pinMode(D4, OUTPUT); // D4 pin as output (Power indicator).
}

void pulse(uint8_t ledid) // This is the breathing LED code
{
